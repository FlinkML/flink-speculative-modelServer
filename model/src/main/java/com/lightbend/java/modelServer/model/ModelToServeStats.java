...
public class ModelToServeStats {
    ...
    private static ModelToServeStats instance = null;
    ...
    public ModelToServeStats(final String name, final String description, String modelType) {
        this.name = name;
    ...
    public ModelToServeStats(ModelToServe model){
        ...
        this.modelType = model.getModelType().toString();
    ...
    public static void writeServingInfo(DataOutputStream output, ModelToServeStats servingInfo) {
        try {
            if(servingInfo == null)
                output.writeLong(0);
            else {
                ...
                output.writeLong(servingInfo.invocations);
    ...
    public static Optional<ModelToServeStats> readServingInfo(DataInputStream input) {

        try{
            long length = input.readLong();
            if(length == 0) return Optional.empty();
            return Optional.of(new ModelToServeStats(input.readUTF(), input.readUTF(), input.readUTF(),
                    input.readLong(), input.readLong(), input.readDouble(), input.readLong(), input.readLong()));
        }
        catch(Throwable t){
