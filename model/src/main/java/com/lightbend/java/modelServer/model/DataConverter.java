...
import com.lightbend.java.modelServer.model.tensorflow.TensorflowModelFactory;
...
public class DataConverter {
    ...
    public static Optional<Cpudata.CPUData> convertData(byte[] binary){
        try {
            // Unmarshall record
            return Optional.of(Cpudata.CPUData.parseFrom(binary));
        } catch (Throwable t) {
            // Oops
    ...
    public static Optional<Speculativedescriptor.SpeculativeDescriptor> convertConfig(byte[] binary){
        try {
            // Unmarshall record
            return Optional.of(Speculativedescriptor.SpeculativeDescriptor.parseFrom(binary));
        } catch (Throwable t) {
            // Oops
            System.out.println("Exception parsing config record" + new String(binary));
            t.printStackTrace();
            return Optional.empty();
        }
    }
    ...
    public static Optional<ModelToServe> convertModel(byte[] binary){
        try {
            ...
            if(model.getMessageContentCase().equals(Modeldescriptor.ModelDescriptor.MessageContentCase.DATA)){
                Modeldescriptor.ModelPreprocessing mp = model.getPreprocessing();
                return Optional.of(new ModelToServe(
                        ...
                        mp.getWidth(), mp.getMean(), mp.getStd(), mp.getInput(), mp.getOutput()));
            }
            else {
    ...
    public static Optional<Speculativedescriptor.SpeculativeDescriptor>fromByteArray(byte[] message) {
        try {
            return Optional.of(Speculativedescriptor.SpeculativeDescriptor.parseFrom(message));
        }
        catch (Throwable t){
            return Optional.empty();
        }
